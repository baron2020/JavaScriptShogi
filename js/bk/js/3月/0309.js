...
console.log(GameRecord[currentMasu]);//直前に動かした駒のId
console.log(MovePossible);//移動可能マスを表示する
...
console.log(GameRecord[currentMasu]);//直前の指し手のマス
		console.log(addMasu);//直前の指し手のマス
		console.log(GameRecord[addMasu]);//直前の指し手のマス
	}
...
tempPiece=firstTouchPiece.substr(0,2);//最初に選択した駒のId二文字

	let indexNumber;
	let a,b,temp;
		indexNumber=CheckPieceId.indexOf(tempPiece);
		...
		for(let i=0;i<sPieceMotionYX.length;i++){
			...
			if(temp>=1){
				a=sPieceMotionYX[i][0];
				b=sPieceMotionYX[i][1];
