...
//駒の配列(先手,後手)(len40)
var piece=["<p class='skoma'id='OU1'>王</p>","<p class='skoma'id='HI1'>飛</p>","<p class='skoma'id='KA1'>角</p>",
			...
			"<p class='gkoma'id='OU2'>玉</p>","<p class='gkoma'id='HI2'>飛</p>","<p class='gkoma'id='KA2'>角</p>",
...
var sArray=["skoma","後手","gkoma"];
var gArray=["gkoma","先手","skoma"];
var sPromotionArray=["skoma promotion","後手","gkoma promotion"];
var gPromotionArray=["gkoma promotion","先手","skoma promotion"];
var whatNumber;//最初にタッチした駒が、駒配列の何番目にあるか？
var switchArray;
...
console.log("駒クラス："+pieceIdRecord[firstTouchPiece]);//駒Idから駒クラスを返す
	...
	if((isMyPiece())||(!InOut(kys,kxs))||
	   ...
	   ((firstTouchMasuInOut==false)&&(opponentPiece()==true))
		...
		console.log("現在のマスにある駒のクラス:"+komaClass);
		reset();
		return;
	}
	//移動先に相手の駒が存在する。
...
cssRuleJustBefore(justBefore[justBefore.length-1]);
	firstChoiceFlg=true;
...
//選択した駒の移動完了まで
function MoveCommit(){
	let senteArray=["先手",gotejinKeima,gotejinKyouFu,gotejin,gotejin];
	let goteArray=["後手",sentejinKeima,sentejinKyouFu,sentejin,sentejin];
	let switchArraySenteGote;	
...
//ここで成るか成らないかの判定-----------------------------------------------------------------
	if(teban=="先手"){
		switchArraySenteGote=senteArray;
	}else{
		switchArraySenteGote=goteArray;
	}
	while(promotionMoveFlg==false){
		//成り駒でない＆teban＆盤内＆成れる駒
		if((promotionFlg==false)&&((teban==switchArraySenteGote[0]) )&&(firstTouchMasuInOut)&&
	   	   (possiblePromotion.indexOf(firstTouchPiece)!=-1)){
			//桂馬が１,２段目に移動したら強制的に成る//桂馬が８,９段目に移動したら強制的に成る
			if((firstTouchPiece.substr(0,2)=="KE")&&( switchArraySenteGote[1](masuCurrent))){
				promotionMoveFlg=true;
				break;
			}
			//香,歩が１段目に移動したら強制的に成る//香,歩が９段目に移動したら強制的に成る
			if((switchArraySenteGote[2](masuCurrent) )&&
		   	   ((firstTouchPiece.substr(0,2)=="KY")||(firstTouchPiece.substr(0,2)=="FU"))){
				promotionMoveFlg=true;
				break;
			}
			//後手陣である。又は、飛車,角,銀の成り返りである。//先手陣である。又は、飛車,角,銀の成り返りである。
			if((switchArraySenteGote[3](masuCurrent))||
			   ((switchArraySenteGote[4](firstTouchMasu))&&
	...
	pieseInsert(firstTouchPiece,masuCurrent,teban);//駒の追加
...
function pieseInsert(firstTouchPiece,masuCurrent,teban){
	//初期Idから見たクラス,反対の手番,反対のクラス
	let whatNumber=pieceId.indexOf(firstTouchPiece);
	if(whatNumber<=19){
		switchArray=sArray;
	}else{
		switchArray=gArray;
	}
	document.getElementById(masuCurrent).insertAdjacentHTML('afterbegin',piece[whatNumber]);	
	GameRecord[masuCurrent]=firstTouchPiece;//現在のマスに駒を追加する	
	pieceIdRecord[firstTouchPiece]=switchArray[0];
	//駒をとった場合の最初は棋譜に記録しない
	if(getFlg==false){
		gameRecodedisp+=kihu+pieceConvert+"　"//棋譜を記録として残す。
//配列に直前の指し手のマスを格納		
justBefore.push(masuCurrent);//配列に直前の指し手のマスを格納
cssRuleJustBefore(justBefore[justBefore.length-1]);
	}
	//駒Idが格納されている配列と手番が一致していないなら逆のクラスにする
	if(teban==switchArray[1]){
		document.getElementById(firstTouchPiece).className=switchArray[2];
...
function promotionMove(){
	let whatNumber=pieceId.indexOf(firstTouchPiece);
	if(whatNumber<=19){
		switchArray=sPromotionArray;
	}else{
		switchArray=gPromotionArray;
	}
	document.getElementById(masuCurrent).insertAdjacentHTML('afterbegin',promotionPiece[whatNumber]);
	GameRecord[masuCurrent]=firstTouchPiece;//現在のマスに駒を追加する
	pieceIdRecord[firstTouchPiece]=switchArray[0];//昇格クラスにする
	gameRecodedisp+=kihu+pieceConvert+"　"//棋譜を記録として残す。
//配列に直前の指し手のマスを格納		
justBefore.push(masuCurrent);//配列に直前の指し手のマスを格納
cssRuleJustBefore(justBefore[justBefore.length-1]);
...
function opponentPiece(){
	if(((teban=="先手")&&(komaClass=="gkoma"))||((teban=="先手")&&(komaClass=="gkoma promotion"))||
	   ((teban=="後手")&&(komaClass=="skoma"))||((teban=="後手")&&(komaClass=="skoma promotion"))){
		return true;
	}
	return false;
}
...
function sentejin(masuCurrent){
	//正規表現
	let regex1=new RegExp(/d7s[1-9]/);
	let regex2=new RegExp(/d8s[1-9]/);
	let regex3=new RegExp(/d9s[1-9]/);
	let promotion1=regex1.test(masuCurrent);
	let promotion2=regex2.test(masuCurrent);
	let promotion3=regex3.test(masuCurrent);
...
function gotejin(masuCurrent){
	//正規表現
	let regex1=new RegExp(/d1s[1-9]/);
	let regex2=new RegExp(/d2s[1-9]/);
	let regex3=new RegExp(/d3s[1-9]/);
	let promotion1=regex1.test(masuCurrent);
	let promotion2=regex2.test(masuCurrent);
	let promotion3=regex3.test(masuCurrent);
	if((promotion1)||(promotion2)||(promotion3)){
...
function sentejinKeima(masuCurrent){
	//正規表現
	let regex1=new RegExp(/d8s[1-9]/);
	let regex2=new RegExp(/d9s[1-9]/);
	let promotion1=regex1.test(masuCurrent);
	let promotion2=regex2.test(masuCurrent);
	if((promotion1)||(promotion2)){
		return true;
